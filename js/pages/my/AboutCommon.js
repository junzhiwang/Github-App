...
export default class AboutCommon {
  ...
  async updateFavorite(repositories) {
    ...
    for (let i = 0; i < data.length; ++i) {
      ...
      projectModels.push(
        new ProjectModel(
          item,
          Utils.checkFavorite(item, this.favoriteKeys || [])
        )
      );
    }
  ...
  async componentDidMount() {
    if (this.flag_about === FLAG_ABOUT.flag_about) {
      ...
      this.repositories.push(
        await this.dataRepository.fetchRepository(
          this.config.info.currentRepoUrl
        )
      );
  ...
  onFavorite(item, isFavorite) {
    if (isFavorite) {
      this.favoriteDao.saveFavoriteItems(
        item.id.toString(),
        JSON.stringify(item)
      );
    } else {
  ...
  getParallaxRenderConfig(params) {
    ...
    config.renderForeground = () => (
      <View key="parallax-header" style={styles.parallaxHeader}>
        ...
        <Text
          style={[styles.sectionTitleText, { marginLeft: 10, marginRight: 10 }]}
        >
          {params.description}
        </Text>
      </View>
