...
export default class TrendingTab extends Component {
  ...
  onFavorite(item, isFavorite) {
    if (isFavorite) {
      this.props.favoriteDao.saveFavoriteItems(
        item.id.toString(),
        JSON.stringify(item)
      );
    } else {
  ...
  flushFavoriteState() {
    ...
    for (var i = 0; i < this.items.length; ++i) {
      projectModels.push(
        new ProjectModel(
          this.items[i],
          Utils.checkFavorite(this.items[i], this.state.favoriteKeys)
        )
      );
    }
    this.setState({
      ...
      this.dataRepository
        ...
        .then(data => {
          if (
            !data ||
            !data.items ||
            !this.dataRepository.checkDate(data.update_date)
          ) {
        ...
        .then(data => {
          if (data) {
            ...
            DeviceEventEmitter.emit("showToast", "Network data unavailable");
        })
        .catch(err => {
          console.log(err);
        });
